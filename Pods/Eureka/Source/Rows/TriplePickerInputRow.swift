...
open class TriplePickerInputCell<A, B, C>: _PickerInputCell<Tuple3<A, B, C>> where A: Equatable, B: Equatable, C: Equatable {
    ...
    open override func numberOfComponents(in _: UIPickerView) -> Int {
        return 3
    }

    open override func pickerView(_: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        if component == 0 {
    ...
    open override func pickerView(_: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        if component == 0 {
...
open class _TriplePickerInputRow<A: Equatable, B: Equatable, C: Equatable>: Row<TriplePickerInputCell<A, B, C>>, NoValueDisplayTextConformance {
    ...
    public var displayValueForFirstRow: ((A) -> (String)) = { a in String(describing: a) }
    /// Modify the displayed values for the second picker row.
    public var displayValueForSecondRow: ((B) -> (String)) = { b in String(describing: b) }
    /// Modify the displayed values for the third picker row.
    public var displayValueForThirdRow: ((C) -> (String)) = { c in String(describing: c) }
...
public final class TriplePickerInputRow<A, B, C>: _TriplePickerInputRow<A, B, C>, RowType where A: Equatable, B: Equatable, C: Equatable {
    public required init(tag: String?) {
        super.init(tag: tag)
        displayValueFor = { [weak self] tuple in
